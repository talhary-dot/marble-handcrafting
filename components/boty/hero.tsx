...
const FEATURED_PIECES: FeaturedPiece[] = [
  {
    ...
    stoneType: "Carrara White & Bronze Vein",
...
export function Hero() {
  ...
  return (
    <section className="relative min-h-[92vh] flex items-center overflow-hidden bg-gradient-to-b from-[#F4EFEA] via-[#EFE9E0] to-[#E9E1D6] py-20 lg:py-28">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          ...
          <div className="lg:col-span-6 xl:col-span-5 text-center lg:text-left pt-6 lg:pt-0">
            <div 
              className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs uppercase tracking-[0.25em] font-medium mb-6 animate-blur-in"
            ...
            <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl lg:text-[4rem] leading-[1.08] mb-6 text-foreground tracking-tight">
            ...
            <p className="text-base sm:text-lg leading-relaxed mb-8 max-w-lg mx-auto lg:mx-0 text-foreground/80">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 mb-10">
              <Link 
                href="/shop" 
                className="group inline-flex items-center justify-center gap-3 bg-primary text-primary-foreground px-8 py-4 rounded-full text-sm font-medium tracking-wide boty-transition hover:bg-primary/90 shadow-[0_8px_25px_rgba(158,86,50,0.25)] hover:shadow-[0_12px_30px_rgba(158,86,50,0.35)] cursor-pointer"
              >
              ...
              <Link 
                href="#craft-story" 
                className="inline-flex items-center justify-center gap-2 border border-foreground/20 text-foreground px-8 py-4 rounded-full text-sm font-medium tracking-wide boty-transition hover:bg-foreground/5"
              >
                Our Stone Craft
              </Link>
            </div>
            ...
            <div className="pt-6 border-t border-border/60 grid grid-cols-3 gap-4 text-center lg:text-left">
              <div>
                <span className="block font-serif text-xl sm:text-2xl font-bold text-foreground">100%</span>
                <span className="text-[11px] uppercase tracking-wider text-muted-foreground block mt-0.5">Solid Stone</span>
              </div>
              <div>
                <span className="block font-serif text-xl sm:text-2xl font-bold text-foreground">Hand</span>
                <span className="text-[11px] uppercase tracking-wider text-muted-foreground block mt-0.5">Chiseled Finish</span>
              </div>
              <div>
                <span className="block font-serif text-xl sm:text-2xl font-bold text-foreground">Global</span>
                <span className="text-[11px] uppercase tracking-wider text-muted-foreground block mt-0.5">Crate Shipping</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-6 xl:col-span-7 relative flex flex-col items-center">
            ...
            <div className="w-full max-w-lg mb-4 flex items-center justify-between gap-1.5 p-1 rounded-2xl bg-card/80 backdrop-blur-md border border-border/60 shadow-xs">
              {FEATURED_PIECES.map((piece, idx) => (
                <button
                  key={piece.id}
                  type="button"
                  onClick={() => setActiveIndex(idx)}
                  className={`flex-1 py-2 px-3 rounded-xl text-xs font-medium boty-transition text-center truncate ${
                    activeIndex === idx
                      ? "bg-primary text-primary-foreground shadow-sm font-semibold"
                      : "text-foreground/70 hover:text-foreground hover:bg-background/60"
                  }`}
                >
                  <span className="hidden sm:inline text-[10px] opacity-75 mr-1.5">0{idx + 1}.</span>
            ...
            <div className="relative w-full max-w-lg aspect-[4/3] sm:aspect-[16/13] rounded-3xl overflow-hidden border border-primary/25 shadow-[0_25px_60px_-15px_rgba(158,86,50,0.22)] bg-card group">
              ...
              <div className="absolute top-4 left-4 sm:top-5 sm:left-5 z-20">
                <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-background/90 backdrop-blur-md border border-white/20 text-foreground text-xs font-medium shadow-md">
                  ...
                  <span>{activePiece.badge}</span>
                  <span className="text-muted-foreground/60">•</span>
                  <span className="text-primary font-semibold">{activePiece.stoneType}</span>
                </div>
              </div>

              {/* Floating Top-Right: Origin Pill */}
              <div className="absolute top-4 right-4 sm:top-5 sm:right-5 z-20 hidden sm:block">
              ...
              <div className="absolute bottom-4 left-4 right-4 sm:bottom-5 sm:left-5 sm:right-5 z-20">
                <div className="p-4 sm:p-5 rounded-2xl bg-card/95 backdrop-blur-xl border border-border/80 shadow-lg flex items-center justify-between gap-4">
                  ...
                  <Link
                    ...
                    className="flex-shrink-0 inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-primary text-primary-foreground text-xs font-medium hover:bg-primary/90 boty-transition shadow-sm"
                  >
                    <span>Inspect</span>
            ...
            <div className="w-full max-w-lg mt-4 flex items-center justify-between px-2 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                <span className="font-medium text-foreground text-[11px]">5.0 • 140+ Reviews</span>
              </div>

              <div className="flex items-center gap-1.5 text-[11px]">
                <ShieldCheck className="w-3.5 h-3.5 text-primary" />
                <span>Authentic Metamorphic Stone</span>
              </div>
            </div>
